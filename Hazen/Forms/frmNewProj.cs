...
namespace Hazen.Forms
{
    public partial class frmNewProj : System.Windows.Forms.Form
    {
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtLength.Text) || !double.TryParse(txtLength.Text, out double length) || (Convert.ToDouble(txtLength.Text) < 0))
            {
                TaskDialog.Show("Data validation", "Please, fix the dimensions. There are some invalid values.");
                return;
            }

            if (string.IsNullOrWhiteSpace(txtWidth.Text) || !double.TryParse(txtWidth.Text, out double width) || (Convert.ToDouble(txtWidth.Text) < 0))
            {
                TaskDialog.Show("Data validation", "Please, fix the dimensions. There are some invalid values.");
                return;
            }

            if (string.IsNullOrWhiteSpace(txtHeight.Text) || !double.TryParse(txtHeight.Text, out double height) || (Convert.ToDouble(txtHeight.Text) < 0))
            {
                TaskDialog.Show("Data validation", "Please, fix the dimensions. There are some invalid values.");
                return;
            }
